package elmeniawy.eslam.twitterfollowers.utils;

/**
 * PreferencesUtils
 * <p>
 * Created by Eslam El-Meniawy on 14-Mar-18.
 */

public class PreferencesUtils {
    private static final String PACKAGE_NAME = "elmeniawy.eslam.twitterfollowers.";
    public static final String PREF_FILE_NAME = PACKAGE_NAME + "TwitterFollowersPref";
    public static final String KEY_RUN_BEFORE = PACKAGE_NAME + "RunBefore";
    public static final String KEY_LOGGED_IN = PACKAGE_NAME + "LoggedIn";
    public static final String KEY_LANG = PACKAGE_NAME + "Lang";
    public static final String KEY_TOKEN = PACKAGE_NAME + "Token";
    public static final String KEY_SECRET = PACKAGE_NAME + "Secret";
    public static final String KEY_USER_NAME = PACKAGE_NAME + "UserName";
    public static final String KEY_USER_ID = PACKAGE_NAME + "UserId";
}
